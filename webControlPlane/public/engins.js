...
function sentRequestTest(textRequest = "engineStates?stateRightEngine=on&stateLeftEngine=off") {
...
function onclick(e) {
    ...
    // send request
    // sentRequestTest(textRequest = languages)
    // alert("test");
    sentRequestTest(textRequest = e.target.value);
    // sentRequestTest(textRequest = "test")
    // sentEngineState(targetServerRoute, engineName, engineState)
...
function engineListner(EngineHtmlClass) {
    ...
    for (var i = 0; i < EngineHtmlClass.languages.length; i++) {
        ...
        // FormForListening.languages[i].addEventListener("click", sentRequestTest(textRequest = FormForListening.languages[i].value));
        // myForm.languages[i].addEventListener("click", sentRequestTest(textRequest = "testReq"));
